...
// TEST 5: Probar Cart Store
const testCartStore = async () => {
    loading.value = true;
    error.value = null;
    try {
        console.log('🧪 Probando Cart Store...');
        ...
        results.value.cartStore = {
            itemCount: cartStore.itemCount,
            totalAmount: cartStore.totalAmount,
            isEmpty: cartStore.isEmpty
        };
    } catch (err) {
        console.error('❌ Error en Cart Store:', err);
...
// TEST 6: Probar Orders Store
const testOrdersStore = async () => {
    loading.value = true;
    error.value = null;
    try {
        console.log('🧪 Probando Orders Store...');

        await ordersStore.fetchMyOrders();
        console.log('✅ Orders Store:', {
            orders: ordersStore.orders,
            count: ordersStore.orderCount,
            pending: ordersStore.pendingOrders,
            completed: ordersStore.completedOrders
        });

        results.value.ordersStore = {
            orderCount: ordersStore.orderCount,
            pendingCount: ordersStore.pendingOrders.length,
            completedCount: ordersStore.completedOrders.length
        };
    } catch (err) {
        console.error('❌ Error en Orders Store:', err);
        error.value = err.message;
    } finally {
        loading.value = false;
    }
...
// TEST 7: Probar Users Store (Admin)
const testUsersStore = async () => {
    loading.value = true;
    error.value = null;
    try {
        console.log('🧪 Probando Users Store...');
        ...
        console.log('✅ Users Store:', {
            ...
            admins: usersStore.adminCount,
            regular: usersStore.regularUserCount
        });

        results.value.usersStore = {
            userCount: usersStore.userCount,
            adminCount: usersStore.adminCount,
            regularUserCount: usersStore.regularUserCount
        };
    } catch (err) {
        console.error('❌ Error en Users Store:', err);
        error.value = err.message;
    } finally {
        loading.value = false;
    }
...
<template>
    ...
    <button @click="testCartStore" :disabled="loading">
        🛒 Test Cart Store
    </button>

    <button @click="testOrdersStore" :disabled="loading">
        📦 Test Orders Store
    </button>

    <button @click="testUsersStore" :disabled="loading" v-if="authStore.user?.role === 'admin'">
        👥 Test Users Store (Admin)
    </button>
</template>
